...
def decode_boxes(res_codes, anchors, bbox_size=7):
    """Generate box regression from residual codes and anchors

    Args:
        res_codes ([type]): [description]
    """
    offset = res_codes.view(-1, bbox_size)
    offset[:, :3] = offset[:, :3] * anchors[:, 3:6] + anchors[:, :3]
    offset[:, 3:6] = torch.exp(offset[:, 3:6]) * anchors[:, 3:6]
    return offset.view(res_codes.shape)
